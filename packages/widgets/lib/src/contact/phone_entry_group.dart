...
class PhoneEntryGroup extends StatelessWidget {
  ...
  PhoneActionCallback onSelectPhoneEntry;

  /// Constructor
  PhoneEntryGroup(
      {Key key, @required this.phoneEntries, this.onSelectPhoneEntry})
      : super(key: key) {
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> children = <Widget>[];
    phoneEntries.forEach((PhoneEntry entry) {
      children.add(_buildPhoneEntryRow(
        entry: entry,
        showPrimaryStar:
            phoneEntries.indexOf(entry) == 0 && phoneEntries.length > 1,
        theme: theme,
      ));
    });
    return new Row(
